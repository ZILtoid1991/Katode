...
extern(C++, KTextEditor) struct Mark {
public:
 ...
 */
extern(C++, KTextEditor) class Document : KParts.ReadWritePart {
    mixin(Q_OBJECT);
public @nogc:
    /*!
     * Constructor.
     ...
     */
    this(DocumentPrivate impl, const KPluginMetaData data, QObject parent);
    ~this();
extern(C++) @nogc nothrow:
     ...
     */
    View createView(QWidget parent, KTextEditor.MainWindow mainWindow = null);
...
// Q_SIGNALS:
     ...
     * \sa createView()
     */
    final void viewCreated(KTextEditor.Document document, KTextEditor.View view);
...
// Q_SIGNALS:
     ...
     * \sa documentName()
     */
    final void documentNameChanged(KTextEditor.Document document) @QSignal;
     ...
     */
    final void documentUrlChanged(KTextEditor.Document document) @QSignal;
     ...
     */
    final void modifiedChanged(KTextEditor.Document document) @QSignal;
     ...
     */
    final void readWriteChanged(KTextEditor.Document document) @QSignal;
...
// Q_SIGNALS:
     ...
     *
     */
    final void documentSavedOrUploaded(KTextEditor.Document document, bool saveAs) @QSignal;
     ...
     *
     */
    final void aboutToClose(KTextEditor.Document document) @QSignal;
     ...
     *
     */
    final void aboutToReload(KTextEditor.Document document) @QSignal;
     ...
     * \since 4.6
     */
    final void reloaded(KTextEditor.Document document) @QSignal;
     ...
     * \since 5.91
     */
    final void aboutToSave(KTextEditor.Document document) @QSignal;
     ...
     */
    extern(C++, class) struct EditingTransaction
    {
    public:
        /*!
         ...
         */
        // this(const EditingTransaction);

        /*!
         * No assignment operator, no copying around editing transactions.
         */
        // EditingTransaction opAssign(const EditingTransaction);
    ...
    private:
        const EditingTransactionPrivate* d;
    }
    /*!
     ...
     */
    bool isEditingTransactionRunning() const;
...
public:
     ...
     * \sa setText()
     */
    QString text(Range range, bool block = false) const;
     ...
     * \sa setText()
     */
    QChar characterAt(KTextEditor.Cursor position) const;
     ...
     */
    QString wordAt(KTextEditor.Cursor cursor) const;
     ...
     */
    KTextEditor.Range wordRangeAt(KTextEditor.Cursor cursor) const;
     ...
     * \since 5.0
     */
    bool isValidTextPosition(KTextEditor.Cursor cursor) const;
     ...
     * \sa setText()
     */
    QStringList textLines(Range range, bool block = false) const;
     ...
     * \sa lines()
     */
    qsizetype totalCharacters() const;
     ...
     * \sa text()
     */
    bool setText(const ref QString text);
     ...
     * \sa text()
     */
    bool setText(const ref QStringList text);
     ...
     */
    bool insertText(KTextEditor.Cursor position, const ref QString text, bool block = false);
     ...
     * \sa setText(), removeText()
     */
    bool insertText(KTextEditor.Cursor position, const ref QStringList text, bool block = false);
     ...
     */
    bool replaceText(Range range, const ref QString text, bool block = false);
     ...
     * \sa setText(), removeText(), insertText()
     */
    bool replaceText(Range range, const ref QStringList text, bool block = false);
     ...
     */
    bool removeText(Range range, bool block = false);
     ...
     * \sa insertText()
     */
    bool insertLine(int line, const ref QString text);
     ...
     * \sa insertText()
     */
    bool insertLines(int line, const ref QStringList text);
     ...
     * \since 5.11
     */
    final QList!KTextEditor.Range searchText(KTextEditor.Range range, const ref QString pattern,
            const SearchOptions options = SearchOptions.Default) const;
     ...
     * \since 6.0
     */
    qsizetype cursorToOffset(KTextEditor.Cursor c) const;
     ...
     * \since 6.0
     */
    KTextEditor.Cursor offsetToCursor(qsizetype offset) const;
    /*
     * SIGNALS
     * Following signals should be emitted by the document if the text content
     * is changed.
...
// Q_SIGNALS:
     ...
     *
     */
    final void editingStarted(KTextEditor.Document document) @QSignal;
     ...
     * \sa textChanged()
     */
    final void editingFinished(KTextEditor.Document document) @QSignal;
     ...
     *
     */
    final void lineWrapped(KTextEditor.Document document, KTextEditor.Cursor position) @QSignal;
     ...
     */
    final void lineUnwrapped(KTextEditor.Document document, int line) @QSignal;
     ...
     *
     */
    final void textInserted(KTextEditor.Document document, KTextEditor.Cursor position, const ref QString text) @QSignal;
     ...
     *
     */
    final void textRemoved(KTextEditor.Document document, KTextEditor.Range range, const ref QString text) @QSignal;
     ...
     * \sa text(), textLine()
     */
    final void textChanged(KTextEditor.Document document) @QSignal;
...
// public:
     ...
     */
    KSyntaxHighlighting.Theme.TextStyle defaultStyleAt(KTextEditor.Cursor position) const;
     ...
     */
    QString highlightingMode() const;
     ...
     */
    QString highlightingModeAt(KTextEditor.Cursor position);
     ...
     */
    QStringList highlightingModes() const;
     ...
     */
    bool setMode(const ref QString name);
     ...
     */
    bool setHighlightingMode(const ref QString name);
     ...
     *
     */
    QString highlightingModeSection(int index) const;
     ...
     *
     */
    QString modeSection(int index) const;
...
// Q_SIGNALS:
     ...
     * \sa setMode()
     */
    final void modeChanged(KTextEditor.Document document) @QSignal;
     ...
     */
    final void highlightingModeChanged(KTextEditor.Document document) @QSignal;
...
// public:
     ...
     */
    void readSessionConfig(const ref KConfigGroup config, const ref QSet!QString flags);
     ...
     */
    void writeSessionConfig(ref KConfigGroup config, const ref QSet!QString flags);
...
// public:
     ...
     */
    bool isDataRecoveryAvailable() const;
...
// Q_SIGNALS:
     ...
     * \since 5.79
     */
    final void configChanged(KTextEditor.Document document) @QSignal;
...
// public:
     ...
     */
    MovingCursor newMovingCursor(KTextEditor.Cursor position, MovingCursor.InsertBehavior insertBehavior =
            MovingCursor.MoveOnInsert);
     ...
     */
    MovingRange newMovingRange(Range range, MovingRange.InsertBehaviors insertBehaviors = MovingRange.DoNotExpand,
     ...
     *
     */
    void transformCursor(ref KTextEditor.Cursor cursor, KTextEditor.MovingCursor.InsertBehavior insertBehavior, qint64 fromRevision, qint64 toRevision = -1);
     ...
     *
     */
    void transformCursor(ref int line, ref int column, KTextEditor.MovingCursor.InsertBehavior insertBehavior, qint64 fromRevision, qint64 toRevision = -1);
     ...
     *
     */
    void transformRange(ref KTextEditor.Range range,
     ...
     */
    final void aboutToInvalidateMovingInterfaceContent(KTextEditor.Document document);
     ...
     */
    QVariant configValue(const ref QString key);

    /*!
     * \brief Set a the \a key's value to \a value.
     */
    void setConfigValue(const ref QString key, const ref QVariant value);
     ...
     */
    final void modifiedOnDisk(KTextEditor.Document document, bool isModified,
            KTextEditor.Document.ModifiedOnDiskReason reason) @QSignal;
     ...
     */
    const ref QHash!(int, KTextEditor.Mark) marks();
     ...
     */
    void setMarkDescription(MarkTypes mark, const ref QString text);
     ...
     */
    QString markDescription(MarkTypes mark) const;
     ...
     * \sa setMarkDescription()
     */
    void setMarkIcon(MarkTypes markType, const ref QIcon icon);
     ...
     * \sa setMarkDescription()
     */
    QIcon markIcon(MarkTypes markType);
     ...
     * \sa markChanged()
     */
    final void marksChanged(KTextEditor.Document document) @QSignal;
     ...
     * \sa marksChanged()
     */
    final void markChanged(KTextEditor.Document document, KTextEditor.Mark mark,
            KTextEditor.Document.MarkChangeAction action) @QSignal;
     ...
     *
     */
    final void markToolTipRequested(KTextEditor.Document document, KTextEditor.Mark mark, QPoint position,
            ref bool handled) @QSignal;
     ...
     *
     */
    final void markContextMenuRequested(KTextEditor.Document document, KTextEditor.Mark mark, QPoint pos,
            ref bool handled) @QSignal;
     ...
     *
     */
    final void markClicked(KTextEditor.Document document, KTextEditor.Mark mark, ref bool handled) @QSignal;
     ...
     */
    AnnotationModel annotationModel() const;
    //!\}

private:
    const DocumentPrivate d;
