module KTextEditor.application;
 ...
 */
extern(C++, KTextEditor) class Application : QObject {
    mixin(Q_OBJECT);

extern(C++) public @nogc nothrow:
    /*!
...
// public:
     ...
     */
    final QList!KTextEditor.MainWindow mainWindows();
     ...
     */
    final KTextEditor.MainWindow activeMainWindow();
...
// public:
     ...
     */
    final QList!KTextEditor.Document documents();
     ...
     */
    final KTextEditor.Document findUrl(const ref QUrl url);
     ...
     */
    final KTextEditor.Document openUrl(const ref QUrl url, const ref QString encoding);
     ...
     */
    final bool closeDocument(KTextEditor.Document document);
     ...
     * Returns \e true on success, otherwise \e false
     */
    final bool closeDocuments(const ref QList!KTextEditor.Document documents);
...
// Q_SIGNALS:
     ...
     */
    final void documentCreated(KTextEditor.Document document) @QSignal;
     ...
     */
    final void documentWillBeDeleted(KTextEditor.Document document) @QSignal;
     ...
     */
    final void documentDeleted(KTextEditor.Document document) @QSignal;
...
// public:
     ...
     */
    final KTextEditor.Plugin plugin(const ref QString name);
...
// Q_SIGNALS:
     ...
     *
     */
    final void pluginCreated(const ref QString name, KTextEditor.Plugin plugin) @QSignal;
     ...
     * Use the pointer only to remove mappings in hash or maps
     */
    final void pluginDeleted(const ref QString name, KTextEditor.Plugin plugin) @QSignal;

private:
    // friend class ApplicationPrivate;

    const ApplicationPrivate d;
}
